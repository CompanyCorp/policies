...
function App() {
  const [cookies, setCookie] = useCookies([
    "specialization",
    "specialization-type",
  ]) as [SpecCookie, (name: string, value: string) => void];

  const [specType, setSpecType] = useState<SpecType>(
    cookies["specialization-type"] || SpecType.POWER,
  );
  const [activeSpec, setSpec] = useState<Specs>(
    cookies["specialization"] || Specs.SLB,
  );
            ...
            <Layout>
              <Switch>
                <Route path={`${CMId.Nightmare}`} nest>
                  <Switch>
                    <Route
                      path={`/${NightmareFight.MAMA}`}
                      component={FightPage}
                    />
                    <Route
                      path={`/${NightmareFight.Siax}`}
                      component={FightPage}
                    />
                    <Route
                      path={`/${NightmareFight.Ensolyss}`}
                      component={FightPage}
                    />
